...
class User(Model):
    id_user = fields.IntField(pk=True, index=True)
...
class Business(Model):
    id_business = fields.IntField(pk=True, index=True)
...
class Product(Model):
    id_product = fields.IntField(pk=True, index=True)
...
# Here we are creating Pydantic models that will be used for API data validation
...
# For each entity, we are creating In and Out models to support requests and responses
